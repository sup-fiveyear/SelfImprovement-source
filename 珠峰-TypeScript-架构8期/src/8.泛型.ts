...
let r = createArray<string>(3, "ABC"); // 使用泛型定义
let r2 = createArray(3, "ABC"); // 不传入类型 ts 也会自动推到类型

// 2.泛型可以使用多个
// A,B 代表变量类型
// function swap<A,B>(arr:[A,B]):([B,A]){
//      return [arr[1],arr[0]]
// }
// swap<string,string>(['string','string'])

// 3. 函数表达式的写法
// 写到函数上的泛型表示调用函数时 传入具体类型
// interface IMySwap {
//     <A, B> (tuple: [A, B]): [B, A]
// }
// const swapFunc: IMySwap = <A,B>(tuple:[A,B]):[B,A] => {
...
//   (tuple: [A, B]): [B, A];
// }
// const swapFunc: IMySwap<string, string> = <A, B>(tuple: [A, B]): [B, A] => {
//   return [tuple[1], tuple[0]];
// };
...
// extends不是继承，只是表示T满足WithLen接口的条件
